const global = {
  ...
  api:{
    apiKey:"caff9051df40f4355b9add45b8d14403",
...
//create and display pagination for seach
function displayPagination(){
  ...
  <button class="btn btn-primary" id="prev">Prev</button>
          ...
          //disable next button if ther is no page
...
// ffetch data from TMDB api
async function fetchApiData(endpoint){
 const API_Key= global.api.apiKey;
 const API_URL = global.api.apiUrl;

 showSpinner();
 const respons = await fetch(`${API_URL}${endpoint}?api_key=${API_Key}&language=en-US`);
...
async function searchApiData() {
  ...
  const response = await fetch(
    `${API_URL}search/${global.search.type}?api_key=${API_KEY}&language=en-US&query=${global.search.term}&page=${global.search.page}&page=${global.search.page}`
  );
